...
public class WaitHelper {
    ...
    public WaitHelper waitForElementToBeVisible(WebElement element) {
        try {
            log.info("Wait for element to be visible " + element.toString());
    ...
    public WaitHelper waitForElementToBeClickable(WebElement element) {
        try {
            log.info("Wait for element to be clickable " + element.toString());
            new WebDriverWait(getDriver(), DEFAULT_TIMEOUT)
    ...
    public WaitHelper waitForUrlChangedFrom(final String oldUrl) {
        log.info("Waiting for URL chang from " + oldUrl);
        new WebDriverWait(getDriver(), DEFAULT_TIMEOUT)
    ...
    public WaitHelper waitForElementToBeNotVisible(WebElement element) {
        try {
            log.info("Wait for element to be visible " + element.toString());
            new WebDriverWait(getDriver(), DEFAULT_TIMEOUT)
